...
async function seedSpaces() {
  try {
    console.log('Starting to seed sample spaces...');
    ...
    console.log(`Using user ${ownerId} as owner for all spaces`);
    ...
    for (const spaceData of sampleSpaces) {
      try {
        ...
        if (spaceError) {
          console.error(`Error creating space "${spaceData.title}":`, spaceError.message);
          continue;
        }
        ...
        if (spaceData.amenities) {
          ...
          if (amenitiesError) {
            console.error(`  - Error adding amenities:`, amenitiesError.message);
          } else {
        ...
        if (spaceData.images && spaceData.images.length > 0) {
          const imageRecords = spaceData.images.map((img, index) => ({
            space_id: space.id,
            image_url: img.url,
            is_primary: index === 0,
            display_order: index
          ...
          if (imagesError) {
            ...
            console.log(`  - Added ${spaceData.images.length} image(s)`);
          }
        }

        successCount++;
      } catch (err) {
        console.error(`Error processing space "${spaceData.title}":`, err);
      }
    }

    console.log(`\n✓ Successfully created ${successCount} out of ${sampleSpaces.length} spaces!`);
    console.log('\nYou can now view them at: http://localhost:3000/spaces');
